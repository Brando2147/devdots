...

{
  ...
  services.cron = {
    ...
    systemCronJobs = [
      ...
      "0 4 * * *  root    zfs send -i rpool/data@`date -d -1days +\\%Y-\\%m-\\%d` rpool/data@`date +\\%Y-\\%m-\\%d` | ssh brando@192.168.0.223 zfs recv rpool/backups | logger -t cron"
      "0 4 * * *  root    ssh brando@192.168.0.223 zfs destroy rpool/data@`date -d -30days +\\%Y-\\%m-\\%d` | logger -t cron"
    ];
